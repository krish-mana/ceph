...
protected:
  ...
  map<uint64_t, set<hobject_t> > snap_to_hobject;
  map<hobject_t, set<uint64_t> > hobject_to_snap;
  uint64_t next;
public:
  ...
  void choose_random_snaps(int num, set<uint64_t> *snaps) {
    assert(snaps);
  ...
  void create_object() {
    ...
    set<uint64_t> &snaps = hobject_to_snap[obj];
    choose_random_snaps(1 + (rand() % 20), &snaps);
    for (set<uint64_t>::iterator i = snaps.begin();
	 i != snaps.end();
	 ++i) {
      map<uint64_t, set<hobject_t> >::iterator j = snap_to_hobject.find(*i);
  ...
  void trim_snap() {
    if (snap_to_hobject.empty())
      return;
    map<uint64_t, set<hobject_t> >::iterator snap =
    ...
    while (mapper->get_next_object_to_trim(snap->first, &hoid) == 0) {
      ...
      map<hobject_t, set<uint64_t> >::iterator j =
	hobject_to_snap.find(hoid);
      assert(j->second.count(snap->first));
      set<uint64_t> old_snaps(j->second);
