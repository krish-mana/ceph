			     ...
			     OutTransaction *out)
{
  ...
  boost::scoped_ptr<ObjectContents> contents(get_current_content(obj));

  interval_set<uint64_t> interval_to_write;
  if (contents->exists()) {
    uint64_t offset = rand() % (SIZE/2);
    uint64_t len = rand() % (SIZE/2);
    if (!len) len = 10;
    interval_to_write.insert(offset, len);
    contents.reset(
      new CloneRange(RandomWriteFull(rand(), offset + len), 
		     *contents,
		     interval_to_write)
      );
  } else {
    uint64_t new_size = SIZE + (rand() % SIZE);
    interval_to_write.insert(0, new_size);
    contents.reset(new RandomWriteFull(rand(), new_size));
  }
  bufferlist to_write;
  ObjectContents::iterator iter = contents->get_iterator();
  iter->seek_to(interval_to_write.range_start());
  for (uint64_t i = interval_to_write.range_start();
       i < interval_to_write.range_end();
       ++i, iter->next()) {
    to_write.append(iter->get());
  }
  out->t->write(coll_t(obj.first),
		hobject_t(sobject_t(obj.second, CEPH_NOSNAP)),
		interval_to_write.range_start(),
		interval_to_write.begin().get_len(),
		to_write);
...
void FileStoreTracker::clone_range(const pair<string, string> &from,
  ...
  to_contents.reset(
    new CloneRange(*from_contents, *to_contents, interval_to_clone)
    );
...
void FileStoreTracker::verify(const string &coll, const string &obj) {
  ...
  for (uint64_t i = valid_reads.first;
       ...
       ++i) {
    boost::scoped_ptr<ObjectContents> old_contents(
      get_content(make_pair(coll, obj), i));
