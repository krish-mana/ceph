...
TEST_F(StoreTest, NTMapTest) {
  coll_t cid("blah");
  hobject_t hoid("testmap", "", CEPH_NOSNAP, 0);
  int r;
  {
    ObjectStore::Transaction t;
    t.create_collection(cid);
  ...
  map<string, bufferlist> attrs;
  {
    ObjectStore::Transaction t;
    t.touch(cid, hoid);
    t.tmap_clear(cid, hoid);
    map<string, bufferptr> start_set;
    t.tmap_setkeys(cid, hoid, start_set);
    store->apply_transaction(t);
  }
  ...
  for (int i = 0; i < 100; i++) {
    ...
    r = store->tmap_get(cid, hoid, &bl, &cur_attrs);
    ASSERT_EQ(r, 0);
    ...
    bl.append(bp);
    map<string, bufferptr> to_add;
    to_add.insert(pair<string, bufferptr>("key-" + string(buf), bp));
    attrs.insert(pair<string, bufferlist>("key-" + string(buf), bl));
    t.tmap_setkeys(cid, hoid, to_add);
    store->apply_transaction(t);
  }
  ...
  while (attrs.size()) {
    ...
    t.tmap_rmkeys(cid, hoid, keys_to_remove);
    store->apply_transaction(t);
