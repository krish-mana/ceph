...
void pg_history_t::encode(bufferlist &bl) const
{
  ...
  ::encode(last_deep_scrub_stamp, bl);
  ::encode(last_clean_scrub_stamp, bl);
  ::encode(first_interval_readable, bl);
  ENCODE_FINISH(bl);
}
...
void pg_history_t::decode(bufferlist::iterator &bl)
{
  ...
  if (struct_v >= 7) {
    ::decode(first_interval_readable, bl);
  }
  DECODE_FINISH(bl);
}
...
void pg_history_t::dump(Formatter *f) const
{
  ...
  f->dump_unsigned("first_interval_readable", first_interval_readable);
...
void pg_history_t::generate_test_instances(list<pg_history_t*>& o)
{
  ...
  o.back()->first_interval_readable = 4;
...
void pg_notify_t::generate_test_instances(list<pg_notify_t*>& o)
{
  ...
  o.push_back(new pg_notify_t(shard_id_t(3), shard_id_t::NO_SHARD, 1, 1, pg_info_t(), t, now));
  t[1] = now + utime_t(1, 1);
  t[2] = now + utime_t(2, 2);
  o.push_back(new pg_notify_t(shard_id_t(0), shard_id_t(0), 3, 10, pg_info_t(), t, now));
}
