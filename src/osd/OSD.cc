		       ...
		       ostream& ss)
{
  ...
  if (command == "dump_ops_in_flight") {
    ...
    osd_lock.Lock();
    // scan pg's
    for (hash_map<spg_t,PG*>::iterator it = pg_map.begin();
	 it != pg_map.end();
	 ++it) {
...
int OSD::shutdown()
{
  ...
  // Shutdown PGs
  for (hash_map<spg_t, PG*>::iterator p = pg_map.begin();
       p != pg_map.end();
       ++p) {
...
void OSD::build_past_intervals_parallel()
{
  ...
  for (hash_map<spg_t, PG*>::iterator i = pg_map.begin();
       i != pg_map.end();
       ++i) {
    PG *pg = i->second;
 ...
 */
void OSD::calc_priors_during(spg_t pgid, epoch_t start, epoch_t end, set<int>& pset)
{
...
void OSD::do_command(Connection *con, tid_t tid, vector<string>& cmd, bufferlist& data)
{
  ...
  // 'tell <pgid>' (which comes in without any of that prefix)?
    ...
    std::set <spg_t> keys;
    for (hash_map<spg_t, PG*>::const_iterator pg_map_e = pg_map.begin();
    ...
    for (std::set <spg_t>::iterator p = keys.begin();
	 p != keys.end(); ++p) {
      hash_map<spg_t, PG*>::iterator q = pg_map.find(*p);
      assert(q != pg_map.end());
      PG *pg = q->second;
      pg->lock();
...
void OSD::advance_map(ObjectStore::Transaction& t, C_Contexts *tfin)
{
  ...
  // scan pg creations
  hash_map<spg_t, create_pg_info>::iterator n = creating_pgs.begin();
  while (n != creating_pgs.end()) {
    hash_map<spg_t, create_pg_info>::iterator p = n++;
    spg_t pgid = p->first;
