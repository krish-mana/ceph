...
void ReplicatedPG::execute_ctx(OpContext *ctx)
{
  ...
  ctx->register_on_applied(
    [m, ctx, this](){
      ...
      if (waiting_for_ack.count(ctx->at_version)) {
	assert(waiting_for_ack.begin()->first == ctx->at_version);
	for (list<pair<OpRequestRef, version_t> >::iterator i =
	       waiting_for_ack[ctx->at_version].begin();
	     i != waiting_for_ack[ctx->at_version].end();
	     ++i) {
	...
	waiting_for_ack.erase(ctx->at_version);
      }
  ...
  ctx->register_on_commit(
	...
	log_op_stats(
      ...
      if (waiting_for_ondisk.count(ctx->at_version)) {
	assert(waiting_for_ondisk.begin()->first == ctx->at_version);
	for (list<pair<OpRequestRef, version_t> >::iterator i =
	       waiting_for_ondisk[ctx->at_version].begin();
	     i != waiting_for_ondisk[ctx->at_version].end();
	     ++i) {
	  osd->reply_op_error(i->first, 0, ctx->at_version,
			      i->second);
	}
	waiting_for_ondisk.erase(ctx->at_version);
      }
  ...
  RepGather *repop = new_repop(ctx, obc, rep_tid);  // new repop claims our obc refs
...
ReplicatedPG::OpContextUPtr ReplicatedPG::trim_object(const hobject_t &coid)
{
  ...
  if (!ctx->lock_manager.get_snaptrimmer_write(
    ...
    ctx->lock_manager.put_locks(
      &to_wake, &requeue_recovery, &requeue_snaptrimmer);
    assert(to_wake.empty());
    assert(!requeue_recovery);
    assert(!requeue_snaptrimmer);
    return NULL;
  }
...
void ReplicatedPG::make_writeable(OpContext *ctx)
{
  ...
  if ((ctx->obs->exists && !ctx->obs->oi.is_whiteout()) && // head exist(ed)
    ...
    t->append(ctx->op_t.release() /* append takes ownership */);
    ctx->op_t.reset(t);
...
{
  RepGather *repop = new_repop(ctx.release(), ctx->obc, ctx->reqid.tid);
  dout(20) << __func__ << " " << repop << dendl;
  issue_repop(repop);
  eval_repop(repop);
  repop->put();
  ...
  bool oid_existed)
{
  ...
  if (ssc) {
    ...
    // caller should have ensured that head and snapdir are not missing
    // so EAGAIN cannot happen.  ENOENT is fine, the default constructor
    // for SnapSet will have set up the state correctly
    assert(r == 0 || r == -ENOENT);
    return ssc;
  }
}
