...
void ReplicatedPG::wait_for_missing_object(const hobject_t& soid, OpRequestRef op)
{
  ...
  if (p != pulling.end()) {
    ...
    recover_missing(soid, v, g_conf->osd_client_op_priority, h);
    pgbackend->run_recovery_op(h, g_conf->osd_client_op_priority);
  }
  ...
  ObjectStore::Transaction *t)
{
  ...
  if (complete) {
    ...
    get_parent()->on_local_recover(
      hoid, object_stat_sum_t(), pi.recovery_info, t);
    return false;
  } else {
...
int ReplicatedPG::recover_primary(int max, ThreadPool::TPHandle &handle)
{
  ...
  while (p != missing.rmissing.end()) {
	...
	int r = recover_missing(
	  soid, need, g_conf->osd_recovery_op_priority, h);
	switch (r) {
	case PULL_YES:
	  ++started;
	  break;
