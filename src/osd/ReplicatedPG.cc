...
ReplicatedPG::RepGather *ReplicatedPG::trim_object(const hobject_t &coid)
{
  ...
  if (new_snaps.empty()) {
    ...
    dout(10) << coid << " snaps " << old_snaps
	     << " -> " << new_snaps << dendl;
    coi.snaps = vector<snapid_t>(new_snaps.begin(), new_snaps.end());
  ...
  ostream &out)
{
  ...
  map<string, bufferptr>::const_iterator i = attrs.find(OI_ATTR);
  if (i == attrs.end()) {
    out << "osd " << osd << " could not find OI attr on object "
	<< hoid << std::endl;
    return true;
  }
  ...
  try {
    ...
    out << "osd " << osd << " could not decode OI attr"
	<< std::endl;
    return true;
  }

  set<snapid_t> snaps(oi.snaps.begin(), oi.snaps.end());
  set<snapid_t> mapped_snaps;
  int r = snap_mapper.get_snaps(hoid, &mapped_snaps);
  if (r == -ENOENT) {
    out << "osd " << osd << " snap_mapper does not have snaps for object "
	<< hoid << " should be "
	<< snaps << std::endl;
    return true;
  }

  if (mapped_snaps != snaps) {
    out << "osd " << osd << " snap_mapper has incorrect snaps for object "
	<< hoid
	<<": mapped is " << mapped_snaps
	<< ", should be " << snaps
	<< std::endl;
    return true;
  }
