...
void PG::calc_ec_acting(
  ...
  map<shard_id_t, set<pg_shard_t> > non_up_acting_by_shard;
  unsigned usable = 0;
  for(map<pg_shard_t, pg_info_t>::const_iterator i = all_info.begin();
      i != all_info.end();
      ++i) {
    if (std::find(acting.begin(), acting.end(), i->first.osd) != acting.end() &&
        std::find(up.begin(), up.end(), i->first.osd) != up.end()) {
      non_up_acting_by_shard[i->first.shard].insert(i->first);
    }
  }
  for (shard_id_t i = 0; i < want.size(); ++i) {
	...
	auth_log_shard->second.log_tail) {
      ...
      for (set<pg_shard_t>::iterator j = non_up_acting_by_shard[i].begin();
	   j != non_up_acting_by_shard[i].end();
	   ++i) {
	assert(j->shard == i);
...
bool PG::choose_acting(pg_shard_t &auth_log_shard_id)
{
  ...
  if (want != acting) {
    dout(10) << "choose_acting want " << want_acting << " != acting " << acting
...
void PG::split_into(pg_t child_pgid, PG *child, unsigned split_bits)
{
  ...
  get_osdmap()->pg_to_up_acting_osds(
    child->info.pgid.pgid, &child->up, &up_primary, &child->acting, &primary);
  child->init_primary(up_primary, primary);
  ...
  ObjectStore::Transaction *t)
{
  ...
  acting = newacting;
  up = newup;
  init_primary(new_up_primary, new_acting_primary);
			    ...
			    ostream &errorstream)
{
  ...
  for (set<hobject_t>::const_iterator k = master_set.begin();
	...
	enum PG::error_type error = _compare_scrub_objects(auth->second->objects[*k],
	  ...
	  errorstream << info.pgid << " replica " << j->first
