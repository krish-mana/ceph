 ...
 public:
  ...
  bool is_acting(pg_shard_t osd) const {
    if (pool.info.ec_pool()) {
      assert(acting.size() > osd.shard);
      return acting[osd.shard] == osd.osd;
    } else {
  ...
  void repair_object(
    ...
    pg_shard_t bad_peer,
    pg_shard_t ok_peer);
  map<pg_shard_t, ScrubMap *>::const_iterator _select_auth_object(
    const hobject_t &obj,
...
#define TrivialEvent(T) struct T : boost::statechart::event< T > { \
 ...
 public:
  ...
  void init_primary(int new_up_primary, int new_acting_primary) {
    if (!pool.info.ec_pool()) {
