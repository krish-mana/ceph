...
bool BackoffThrottle::set_params(
  ...
  double _expected_delay_per_count,
  double _max_delay_per_count,
  ...
  ostream *errstream)
{
  ...
  if (_max_delay_per_count < _expected_delay_per_count) {
    valid = false;
    if (errstream) {
      *errstream << "expected_delay (" << _expected_delay_per_count
		 << ") > max_delay (" << _max_delay_per_count
		 << ")" << std::endl;
    }
  }
  ...
  if (_max_delay_per_count < 0) {
    valid = false;
    if (errstream) {
      *errstream << "invalid max_delay_per_count ("
		 << _max_delay_per_count << ")"
		 << std::endl;
    }
  }

  if (_expected_delay_per_count < 0) {
    valid = false;
    if (errstream) {
      *errstream << "invalid expected_delay ("
		 << _expected_delay_per_count << ")"
		 << std::endl;
    }
  }
  ...
  high_threshhold = _high_threshhold;
  expected_delay_per_count = _expected_delay_per_count;
  max_delay_per_count = _max_delay_per_count;
  max = _throttle_max;

  if (high_threshhold - low_threshhold > 0) {
    s0 = 2 * (expected_delay_per_count / (high_threshhold - low_threshhold));
  } else {
  ...
  if (1 - high_threshhold > 0) {
    s1 = (max_delay_per_count - (2 * expected_delay_per_count))
      / (1 - high_threshhold);
  } else {
    high_threshhold = 1;
    s1 = 0;
...
std::chrono::duration<double> BackoffThrottle::_get_delay(uint64_t c) const
{
  ...
  if (r < low_threshhold) {
    ...
    return c * std::chrono::duration<double>(
      (2 * expected_delay_per_count) + ((r - high_threshhold) * s1));
  }
}
