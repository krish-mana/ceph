...
uint32_t DynamicThrottle::calc_wait_time(uint32_t percentage_empty, 
  ...
  if (last_percentage_empty < percentage_empty) {
    ...
    delay_readjusted++;
    ldout(cct, 10) <<"next_threshold adjusted to "<<next_threshold
      <<" percentage_empty = "<<percentage_empty <<" delay_time = "<<delay_time
      <<" delay_readjusted "<< delay_readjusted << dendl;
  }

  if (percentage_empty < next_threshold && 
    ...
    if (percentage_empty < aggressive_delay_threshold && 
      ...
      delay_time += readjust_delay;
      ldout(cct, 10) <<"Waiting for " <<delay_time <<" readjust_delay = "
            <<readjust_delay<<" percentage_empty = "<<percentage_empty
            <<" last_percentage_empty = "<<last_percentage_empty
            <<" next_threshold "<<next_threshold<<" delay_readjusted = "
