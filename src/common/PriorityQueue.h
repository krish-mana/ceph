...
/**
 * Manages queue for low, high, immediate priority items
 *
 ...
 *
 * enqueue_high and enqueue_high_front queue items into queues
...
public:
  PriorityQueue()
    : lock("PriorityQueue::lock"), total_priority(0) {}
  ...
  void enqueue_immediate(K cl, unsigned priority, T item) {
    high_queue[priority].enqueue(cl, 0, item);
  }

  void enqueue_immediate_front(K cl, unsigned priority, T item) {
