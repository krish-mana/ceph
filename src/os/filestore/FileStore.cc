  ...
  m_filestore_max_inline_xattrs(0)
{
  ...
  if (g_conf->filestore_odsync_write || !g_conf->filestore_wbthrottle_enable) {
    m_disable_wbthrottle = true;
  } else {
    m_disable_wbthrottle = false;
  }
}
...
void FileStore::queue_op(OpSequencer *osr, Op *o)
{
  ...
  dout(5) << "queue_op " << o << " seq " << o->op
	  << " " << *osr
	  << " " << o->bytes << " bytes"
	  << "   (queue has " << m_filestore_current_queue_ops.read() << " ops and " 
...
{
  if ((0 != m_filestore_queue_max_bytes) && (0 != m_filestore_queue_max_ops)) {
    ...
    m_filestore_current_queue_ops.add(o->ops);
    uint64_t outstanding_ops = m_filestore_current_queue_ops.read();
    int percentage_empty_ops = 100;
    if (outstanding_ops > max_ops) {
      percentage_empty_ops = 0;

    } else {
      percentage_empty_ops = (uint32_t)((100*(max_ops -
                               outstanding_ops))/max_ops);
    }
    int percentage_empty = 
    (percentage_empty_ops < percentage_empty_bytes)? percentage_empty_ops 
                                                   : percentage_empty_bytes;
    double delay_time;
    uint32_t failed = dyn_throttle_filestore.calc_wait_time_in_sec(percentage_empty
                                                                  , delay_time);
    if ((0 != delay_time) && (!failed)) {
      ...
      dout(10) << " Filestore on BaseDir = " << basedir << " and in journal on "
               ...
               << outstanding_bytes << " Outstanding ops = "
               << outstanding_ops << " seq = " << o->op
               << " Max op threshold = " << max_ops
               << " Max byte threshold = " << max_bytes
               << " Transaction size = " << o->bytes
	       << " Transaction ops = " << o->ops
               << " Percentage empty = " << percentage_empty
               << " Applying delay = " << delay_time << " in the journal commit path."
               << dendl;

    }
    logger->set(l_os_oq_ops, outstanding_ops);
...
void FileStore::op_queue_release_throttle(Op *o)
{
  ...
  m_filestore_current_queue_ops.sub(o->ops);
  logger->set(l_os_oq_ops, m_filestore_current_queue_ops.read());
